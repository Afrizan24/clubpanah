...
<form action="{{ route('admin.beranda.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            @foreach (['hero1', 'hero2'] as $hero)
                <div>
                    ...
                    @endif
                    <input type="file" name="fotos[{{ $hero }}]"
    ...
    <!-- Kegiatan & Program -->
    <div class="bg-white p-6 rounded-xl shadow">
        <h3 class="text-xl font-semibold text-gray-800 border-b pb-2 mb-4">Kegiatan & Program</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="judul_kegiatan" class="block text-sm font-medium text-gray-700 mb-1">Judul</label>
                <input id="judul_kegiatan" name="judul_kegiatan" type="text"
                    class="w-full px-4 py-2 border rounded-lg" value="{{ old('judul_kegiatan', $beranda->judul_kegiatan ?? '') }}">
                @error('judul_kegiatan')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                <label for="deskripsi_kegiatan" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                <textarea id="deskripsi_kegiatan" name="deskripsi_kegiatan" rows="3"
                    class="w-full px-4 py-2 border rounded-lg">{{ old('deskripsi_kegiatan', $beranda->deskripsi_kegiatan ?? '') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @for ($i = 1; $i <= 4; $i++)
                <div>
                    ...
                    @endif
                    <input type="file" name="fotos[galeri{{ $i }}]" class="w-full px-4 py-2 border rounded-lg">
                ...
                <div>
                    ...
                    @if (isset($fotos['about' . $i]) && $fotos['about' . $i]->first())
                        <img src="{{ Storage::url($fotos['about' . $i]->first()->gambar) }}"
                            class="w-full h-32 object-cover rounded mb-2">
                    @endif
                    <input type="file" name="fotos[about{{ $i }}]" class="w-full px-4 py-2 border rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <input id="alamat" name="alamat" type="text"
                    class="w-full px-4 py-2 border rounded-lg" value="{{ old('alamat', $beranda->alamat ?? '') }}">
                @error('alamat')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
