...
<form id="layananForm" method="POST" action="{{ route('admin.layanan.store') }}" enctype="multipart/form-data">
    ...
    @if($layanan->count() < 1)
        ...
        <div class="mb-4">
            ...
            <input type="number" name="biaya" class="w-full p-3 border rounded-lg" placeholder="Contoh: 50000" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="border rounded-lg p-4 shadow">
                <div class="flex items-center mb-3">
                    @if($testimonial->foto)
                        <img src="{{ asset('storage/'.$testimonial->foto) }}" alt="{{ $testimonial->nama }}" class="w-12 h-12 rounded-full object-cover mr-3">
                    @endif
                    <div>
                ...
                <form action="{{ route('admin.layanan.deleteTestimonial', $testimonial->id) }}" method="POST" class="mt-2">
                    @csrf
                    @method('DELETE')
...
<div class="mt-10">
    ...
    <form action="{{ route('admin.layanan.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow">
        @csrf
        ...
        <div class="mb-4">
            ...
            @if(isset($profile) && $profile->logo)
                <div class="mt-2">
                    <img src="{{ asset('storage/' . $profile->logo) }}" alt="Logo" class="w-24 h-auto rounded border">
                </div>
            @endif
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea id="description" name="description" class="w-full p-3 border rounded-lg" rows="4" placeholder="Deskripsi singkat tentang klub...">{{ old('description', $profile->description ?? '') }}</textarea>
        </div>
